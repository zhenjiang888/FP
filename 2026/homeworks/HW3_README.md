...
这次作业需要你安装一些外部包（package）。如你安装 Haskell 环境时所见，Haskell 有 Cabal 和 Stack 两个主要包管理器。本指南提供两个包管理器的换源方式以及基本使用方法，以及如何获知 `random` 包的文档。课上请大家使用 Stack 管理此项目，实际作业中使用任意一者均可。在作业提交上，仅限本次，你可以选择：
- 提交一个 Cabal 项目（压缩包）
- 提交一个 Stack 项目（压缩包）
- 提交单个源文件
...
考虑到你未来可能接触各种语言，建立开发环境的麻烦事逐渐会由大语言模型完成，所以&zwnj;_仅限**本次作业**中的**包管理器和软件包安装**部分_，你可以考虑让大语言模型辅助你。但是，你应该理解“软件包”“依赖”“包管理器”这些跨语言通用的概念。具体而言：
- 你**可以**让大语言模型帮助你安装必要或可选的软件，如 Cabal、Stack、Git。但你必须能简洁明确地描述用到的每个软件的用途（可以要求大语言模型教你，但你必须理解，下同）。
- 你**可以**让大语言模型帮助你建立项目、安装依赖的 `random` 包。但你必须能简洁明确地描述它进行了哪些步骤的操作，每一步做了什么，对文件的修改是什么含义。
- 你**可以**让大语言模型帮助你查询 `random` 包的文档。但你必须能知道仅使用浏览器和搜索引擎而不直接使用大语言模型，相关文档可以在哪里读到。
- **禁止**让大语言模型帮助你写最终的代码。你**不应该**将题目直接发送给大语言模型，而**应该**主动要求大语言模型不要为你编写 Haskell 代码。

如果编写代码部分有极大困难而难以完成，由于第 3 章的内容很大程度上是体验性的，与其直接求助大模型，你也可以按照讲义要求，在提交的作业文件中以注释形式陈述你遇到的困难和做出的努力，并在之后课程中详细学习 Monad 相关内容后，自行尝试重新完成。如果你确实认真地展现你的努力和难处，你并不会因此得不到本次作业分数。
